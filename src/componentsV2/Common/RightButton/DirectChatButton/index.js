...
export default class DirectChatButton extends React.PureComponent {
  ...
  handleResult = (data, jid) => {
    ...
    window.broadcast.send('directImSuccess', successItems.map(R.prop('to_uid')))
  ...
  handleSubmitInvite = (content, jid) => {
    ...
    this.props
      ...
      .then((res) => {
        ...
        this.handleResult(data, jid)
        this.refreshCurrentUser()
        this.props.onInviteFinish(ids, data)
        this.handleCancelInvite()
      })
  }

  handleShowInvite = async (e) => {
    ...
    // if (
    //   auth.isPersonalUser &&
    //   !R.pathOr(0, ['mem', 'fast_contact'], currentUser)
    // ) {
    //   this.props.dispatch({
  ...
  render() {
    ...
    const Chatting = this.props.auth.isTalentBankStable
