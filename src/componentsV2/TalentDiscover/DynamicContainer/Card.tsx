...
import { checkIsTrial, asyncExtraData } from 'utils'
...
@connect((state) => ({
  ...
  dynamicDetaillLoading:
    state.loading.effects['talentDiscover/fetchDynamicDetails'],
}))
export default class Card extends React.Component<Props, State> {
  ...
  handleShowDetailModal = (e) => {
    e.stopPropagation()
    const { mosaic, version } = this.props
    if (version === '3.0' && mosaic) {
      this.handleShowUpgradeMember()
      return
    }
  ...
  renderDetailModal = () => {
    const { trackParam, version = '' } = this.props
    return (
      <DynamicDetailModal
