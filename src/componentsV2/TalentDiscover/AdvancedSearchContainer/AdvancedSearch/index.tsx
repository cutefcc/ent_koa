...
import { INIT_ADVANCE_SEARCH, INIT_ADVANCE_SEARCH_PRO } from 'constants/talentDiscover'
...
export default class Subscription extends React.PureComponent<Props, State> {
  ...
  componentDidMount() {
    ...
    this.props.onChange(this.props.advancedSearch)
  }
  ...
  fetchProfession = () => this.props.dispatch({ type: 'global/fetchProfession' })
  ...
  handleCommonFacetChange = key => value => {
    trackEvent('jobs_pc_talent_discover_search_btn_click')
    this.props.onChange({
      ...this.props.value,
      [key]: value,
    })
  ...
  render() {
    ...
    const advancedSearchJson = JSON.stringify(this.props.advancedSearch)
    const isHiddenOpe = advancedSearchJson !== JSON.stringify(INIT_ADVANCE_SEARCH) && advancedSearchJson !== JSON.stringify(INIT_ADVANCE_SEARCH_PRO)
    return (
      <div className={styles.main}>
        {/* <div className={styles.title}>
          <span onClick={this.handleSaveCondition}>
            <Icon type="download" className="margin-right-5" />保存订阅人才
          </span>
        </div> */}
        ...
        <div className={styles.keywordsSearch}>
          <div className={`${styles.keywordsSearchTop}`}>
            <div className={`${styles.leftSign}`} />
            <div className={`${styles.keywordsText} font-size-16`}>
              <div className={styles.leftText}>搜索关键词</div>
              {isHiddenOpe && <div className={styles.subscriptionOpe}>
                <div className={`${styles.clearSubscription} ${styles.ope}`}>
                  <span onClick={this.props.onClear}>清空</span>
                </div>
                <div className={`${styles.saveSubscription} ${styles.ope}`}>
                  <span onClick={this.props.onSaveSubscription}>保存订阅</span>
                </div>
              </div>}

            </div>
          </div>
          <div className={`${styles.keywordsSearchInput}`}>
            <Search
              placeholder="请输入"
              onSearch={this.handleCommonFacetChange('query')}
              style={{ width: 202 }}
              value={this.props.searchValue || ''}
              onChange={this.props.onSearchValueChange}
              onClick={() => {
                trackEvent('jobs_pc_talent_discover_keywords_click')
              }}
            />
          </div>
        </div>
