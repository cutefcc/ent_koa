...
import EmojiPanel from 'componentsV2/Company/PublishDynamic/EmojiPanel'
...
const CommentDetailItem = (props) => {
  ...
  const { position, career, compos = '无', name, avatar } = u
  const { name: replayRealName } = lv1U
  const { name: ruReplayRealName } = ru
      ...
      <div className={styles.commentDetailItem}>
        ...
        <div className={styles.content}>
          <div className={styles.companyInfo}>
            <div>
              {' '}
              <Text className={styles.companyName}>{name}</Text>
              {lv === 1 && (
                <span>
                  {' '}
                  {compos && '.'}{' '}
                  <Text className={styles.companyNumIcon}>
                    {position || career || compos}
                  </Text>
                </span>
              )}
              {lv === 2 && (
