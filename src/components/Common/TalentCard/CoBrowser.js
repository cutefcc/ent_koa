...
@connect((state) => ({
  loading: state.loading.effects['talents/fetchCoBrowser'],
}))
export default class CoBrowser extends React.Component {
  static propTypes = {
    ...
    return (
      ...
      >
        <h5 className="font-size-16 text-center color-stress font-weight-bold">
          {`${this.props.uname}已被${total}位同事浏览`}
        </h5>
    ...
    return (
      <div key={item.name} className="margin-top-16 flex">
        ...
        <span className="flex-column space-between margin-left-24">
          ...
          <span className="color-diution font-size-14">{item.visit_time}</span>
        </span>
      </div>
    )
  }
  ...
  render() {
    return (
      ...
      >
        ...
        >
          <span className={`like-link-button color-card-footer font-size-14}`}>
            同事浏览·{this.props.total}
          </span>
        </Popover>
      </div>
    )
