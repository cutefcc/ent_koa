...
import styles from './dynamicDetailModal.less'

@connect((state) => ({
  currentUser: state.global.currentUser,
  urlPrefix: state.global.urlPrefix,
  isLoading: state.loading.effects['talentPool/fetchDynamicDetails'],
}))
export default class DynamicDetailModal extends React.Component {
  state = {
    hasMore: true,
    list: [],
    last_time: null,
  ...
  getDetailByEventType = (item, userInfo) => {
    ...
    if (!currentItem.isShowChild && [5, 9].includes(currentItem.event_type)) {
      this.props
        .dispatch({
          type: 'talentPool/fetchDynamicDetailsByEventType',
          payload: {
            to_uid: userInfo.id,
            size: 100,
  ...
  render() {
    ...
    const { name, company, position } = userInfo
    const { list } = this.state
    return (
      <Modal
        title="人才动态明细"
