...
@connect((state) => ({
  ...
  isLoading: state.loading.effects['groups/fetchDynamicDetails'],
}))
export default class DynamicDetailModal extends React.Component<Props, State> {
  ...
  getDetailByEventType = (item, userInfo) => {
    ...
    if (!currentItem.isShowChild && [5, 9].includes(currentItem.event_type)) {
      this.props
        .dispatch({
          type: 'groups/fetchDynamicDetailsByEventType',
          payload: {
            to_uid: userInfo.id,
            size: 100,
  ...
  fetchDynamicDetail = () => {
    ...
    console.log('groups fetchDynamicDetail')
    this.props
      .dispatch({
        type: 'groups/fetchDynamicDetails',
        payload: {
          to_uid: userInfo.id,
          size: pageNum,
